...
class Policy(nn.Module):
    ...
    def load_adj_matrix(self, A):
        self.base.load_adj_matrix(A)
...
class FILMBase(NNBase):
    def __init__(self, num_inputs, film_size=0, hidden_size=64):
        ...
        if film_size > 0:
            self.FILM = init_(nn.Linear(film_size, 2*hidden_size))

        self.train()

    def forward(self, inputs, rnn_hxs, masks):
        x = inputs[:, self.film_size:]
        critic = self.critic(x)
        hidden_actor = self.actor(x)
        if self.film_size > 0:
            a, b = torch.chunk(self.FILM(inputs[:, :self.film_size]), 2, dim=1)
            hidden_actor = hidden_actor * a + b
